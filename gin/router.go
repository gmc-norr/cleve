...
func NewRouter(db *mongo.DB, debug bool) http.Handler {
	...
	r.GET("/api/analyses", AnalysesHandler(db, 0))
	r.GET("/api/runs", RunsHandler(db))
	r.GET("/api/runs/:runId", RunHandler(db))
	r.GET("/api/runs/:runId/analyses", AnalysesHandler(db, cleve.LevelRun))
	r.GET("/api/runs/:runId/analyses/:analysisId", AnalysisHandler(db, cleve.LevelRun))
	...
	r.GET("/api/samples/:sampleId/analyses", AnalysesHandler(db, cleve.LevelSample))
	r.GET("/api/samples/:sampleId/analyses/:analysisId", AnalysisHandler(db, cleve.LevelSample))
	...
	authEndpoints.PATCH("/api/analyses/:parentId/:analysisId", UpdateAnalysisHandler(db, 0))
