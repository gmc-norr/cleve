...
type Run struct {
	...
	SampleSheetFiles []SampleSheetInfo  `bson:"samplesheets" json:"samplesheets"`
...
func (r *Run) UnmarshalBSON(data []byte) error {
	...
	// New structure that we'll be moving to
	ss = rawData.Lookup("samplesheets")
	if len(ss.Value) > 0 {
		err = ss.Unmarshal(&r.SampleSheetFiles)
		if err != nil {
			return err
		}
	}
